...
def get_color_cycle(palette_name: str):
    ...
    if palette_name == "matplotlib (défaut)":
        return plt.rcParamsDefault["axes.prop_cycle"].by_key().get(
            "color",
            [
                "#1f77b4",
                "#ff7f0e",
...
# ------------------------------------------------------------
def laeq_1h_blocks_strict(
    times: pd.Series, levels_db: pd.Series, anchor: pd.Timestamp, T_seconds: float = 3600.0
) -> pd.DataFrame:
    """
...
# --- Gestion du mode Démo & reset de l'uploader ---
if "use_demo" not in st.session_state:
    st.session_state.use_demo = False
if "upload_key" not in st.session_state:
    st.session_state.upload_key = "xlsx_upload_1"  # clé unique pour forcer le reset

# Sélecteur de source toujours visible dans la barre latérale
with st.sidebar.expander("📁 Source des données", expanded=True):
    ...
    new_use_demo = source == "Démo"
    if new_use_demo != st.session_state.use_demo:
        st.session_state.use_demo = new_use_demo
        # Incrémente la clé pour vider l'uploader immédiatement
        k = int(st.session_state.upload_key.rsplit("_", 1)[-1]) + 1
        st.session_state.upload_key = f"xlsx_upload_{k}"
        st.rerun()

# Uploader avec clé pilotée
...
# Message d'aide si aucune donnée et pas en mode Démo
if not st.session_state.use_demo and uploaded_file is None:
    st.info(
        "Importez un fichier Excel **ou** sélectionnez **Démo** dans la barre latérale pour charger `static/DemoBruitAmbiant.xlsx`."
    )
...
with st.sidebar.expander("📊 LAeq 1h"):
    ...
    show_labels = (
        st.checkbox("Afficher la valeur au-dessus de chaque segment", value=False) if show_laeq1h else False
    )
    if show_labels:
...
# 1) Déterminer la source (upload vs démo)
if st.session_state.use_demo:
    demo_path = Path(__file__).parent / "static" / "DemoBruitAmbiant.xlsx"
    if not demo_path.exists():
        st.error(
            f"Le fichier de démo est introuvable : `{demo_path}`.\n"
            "➡️ Vérifiez que le répertoire `static/` contient bien `DemoBruitAmbiant.xlsx`."
        )
    else:
        try:
            df = pd.read_excel(demo_path)
            sourcelabel = f"(Démo) {demo_path.name}"
        except Exception as e:
            st.error(f"Impossible de lire le fichier de démo : {e}")
            df = None
else:
    if uploaded_file is not None:
        try:
            df = pd.read_excel(uploaded_file)
            sourcelabel = f"(Upload) {uploaded_file.name}"
        except Exception as e:
            st.error(f"Impossible de lire le fichier importé : {e}")
            df = None

# 2) Continuer uniquement si des données sont disponibles
if df is None:
    st.stop()  # rien d'autre à afficher
...
# Titre (avec rappel de la source)
source_suffix = f" — {sourcelabel}" if sourcelabel else ""
default_title = f"Données mesurées de {date_debut} à {date_fin}{source_suffix}"
...
if show_laeq1h and laeq1h_mode:
    ...
    for _, r in la1h_plot.iterrows():
        ...
        Rp = min(R, date_fin)
        # Segment LAeq 1h
        ax1.plot([Lp, Rp], [v, v], color=color_laeqh, linewidth=lw, solid_capstyle="butt")
        ...
        if show_labels:
            ...
            ax1.text(
                x_mid, v + label_offset, text_val,
                ha="center", va="bottom",
                fontsize=label_fontsize, color=label_color,
                bbox=bbox, clip_on=True
            )

    # Légende
    ax1.plot([], [], color=color_laeqh, linewidth=lw, label="LAeq 1h")
